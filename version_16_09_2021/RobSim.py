...
scene.lights = []
lamp = local_light(pos=vector(0,1000,-1000),color=color.white)
...
#############################################################################################
    if (serialData.inWaiting()>0): #Start reading data if it is available
        ...
        try:
            ...
            fx = (float(Data[5]))
            fy = (float(Data[6]))
            ang_f0x = float(Data[7])
            ang_f0y = float(Data[8])+10
            ang_f1x = float(Data[9])
            ang_f1y = float(Data[10])
            ang_f2x = float(Data[11])
            ang_f2y = float(Data[12])
            ang_f3x = float(Data[13])
            ang_f3y = float(Data[14])
            ang_f4x = float(Data[15])*2
            ang_f4y = float(Data[16])
        except:
            continue
